...
function generateAIResponse(message) {
    const responses = {
        'привет': 'Привет! Я MichaelASeek 1.0. Чем могу помочь?',
        'как дела': 'Отлично! Готов помочь вам с любыми вопросами.',
        'что ты умеешь': 'Я могу: отвечать на вопросы, генерировать тексты, помогать с задачами, создавать изображения по описанию и многое другое!',
        'спасибо': 'Пожалуйста! Обращайтесь, если понадобится помощь.',
        'пока': 'До свидания! Буду рад помочь снова.',
        'нейросеть': 'Я - MichaelASeek 1.0, продвинутая нейросеть для решения различных задач.',
        'создай изображение': 'Для генерации изображения используйте команду "генерация изображения: [ваше описание]"'
    };
    ...
    if (lowerMessage.includes('погода')) {
        return 'К сожалению, я не могу получить актуальные данные о погоде. Рекомендую проверить специализированные сервисы.';
    }
    ...
    if (lowerMessage.includes('помощь') || lowerMessage.includes('help')) {
        return 'Я могу:\n• Отвечать на вопросы\n• Генерировать тексты\n• Создавать изображения (используйте "генерация изображения: описание")\n• Помогать с различными задачами\nДля поддержки нажмите кнопку "Поддержка"';
    }

    if (lowerMessage.includes('генерация изображения') || lowerMessage.includes('создай картинку')) {
        const prompt = message.replace(/генерация изображения:|создай картинку:/i, '').trim();
        if (prompt) {
            return `🖼️ ГЕНЕРАЦИЯ_ИЗОБРАЖЕНИЯ:${prompt}`;
        }
        return 'Пожалуйста, укажите описание для генерации изображения. Например: "генерация изображения: красивый закат над морем"';
    }
...
// Fake image generation (in real app use DALL-E, Stable Diffusion, etc.)
...
app.post('/api/chat', authenticateToken, (req, res) => {
    ...
    let aiResponse;
    if (message.toLowerCase().includes('генерация изображения') || message.toLowerCase().includes('создай картинку')) {
        const imageResult = generateFakeImage(message.replace(/генерация изображения:|создай картинку:/i, '').trim());
